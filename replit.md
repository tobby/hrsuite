...
- **Performance Appraisals**: Supports 180° and 360° review cycles, template management, and progress tracking. Features include review forms with star ratings and text questions, and results pages with weighted scores and anonymous peer feedback. Admins can edit (name, dates, weights) and delete draft cycles. Cycle deletion cascades to participants, peer assignments, appraisals, feedback, and ratings. API responses for `/api/feedback/:id` and `/api/appraisals/:id` use nested structures matching frontend interfaces.
