...
import { GraduationCap, Inbox, Check, X, Info, Loader2 } from "lucide-react";
...
import type { LdRequest, Employee, Department } from "@shared/schema";
...
function getCourseTypeName(courseType: string) {
...
export default function LdAnalytics() {
  const { role } = useRole();
  const [, navigate] = useLocation();

  if (!canEditOrgSettings(role)) {
    navigate("/ld-requests");
    return null;
  }
  ...
  currentYearRequests.forEach((r) => {
    courseTypeCounts[r.courseType] = (courseTypeCounts[r.courseType] || 0) + 1;
  });
  const topCourseType = Object.entries(courseTypeCounts).sort((a, b) => b[1] - a[1])[0];
  const topCourseTypeName = topCourseType ? getCourseTypeName(topCourseType[0]) : "N/A";

  // Requests by Course Type
  const usageByTypeData = Object.entries(courseTypeCounts).map(([type, count]) => ({
    name: getCourseTypeName(type),
    count,
  }));
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold tracking-tight" data-testid="text-ld-analytics-title">
            L&D Analytics
          </h1>
        </div>
        <p className="text-muted-foreground">
          View learning and development request statistics and trends across the organization
        </p>
      </div>
      ...
      {loadingAll ? (
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-md" data-testid="text-analytics-empty-description">
                L&D analytics will appear here once there are learning and development requests.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card data-testid="card-top-course-type">
              <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Top Course Type</CardTitle>
                <Info className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold" data-testid="text-top-course-type">{topCourseTypeName}</div>
                <p className="text-xs text-muted-foreground">Most requested course type</p>
              </CardContent>
            </Card>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card data-testid="card-requests-by-course-type">
              <CardHeader>
                <CardTitle className="text-base">Requests by Course Type</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-[300px]" data-testid="chart-requests-by-course-type">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={usageByTypeData} barCategoryGap="20%">
                      <defs>
                        <linearGradient id="ldBarGradientBlue" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Bar dataKey="count" fill="url(#ldBarGradientBlue)" radius={[6, 6, 0, 0]} animationDuration={800} animationEasing="ease-out" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
                    ...
                    <PieChart>
                      <defs>
                        {statusData.map((entry, idx) => (
                          <linearGradient key={`ldPieGrad-${idx}`} id={`ldPieGrad-${idx}`} x1="0" y1="0" x2="1" y2="1">
                      ...
                      >
                        {statusData.map((_entry, index) => (
                          <Cell key={`cell-${index}`} fill={`url(#ldPieGrad-${index})`} />
                        ))}
                      </Pie>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="h-[300px]" data-testid="chart-monthly-trends">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={monthlyData}>
                      <defs>
                        <linearGradient id="ldAreaGradient" x1="0" y1="0" x2="0" y2="1">
            ...
            <Card data-testid="card-ld-by-department">
              <CardHeader>
                <CardTitle className="text-base">Requests by Department</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-[300px]" data-testid="chart-ld-by-department">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={deptData} barCategoryGap="20%">
                      <defs>
                        <linearGradient id="ldBarGradientTeal" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} axisLine={false} tickLine={false} allowDecimals={false} />
                      <Tooltip content={<CustomTooltip />} cursor={{ fill: "hsl(var(--muted-foreground))", fillOpacity: 0.06 }} />
                      <Bar dataKey="count" fill="url(#ldBarGradientTeal)" radius={[6, 6, 0, 0]} animationDuration={800} animationEasing="ease-out" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
